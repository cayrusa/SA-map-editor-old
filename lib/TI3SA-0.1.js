...
function fileloaded(e) {
    ...
    if ("image" == e.item.type)
        ...
        case"CC":
            ...
            cPanel.panels.CC.tiles.push(counter);
            break;
        case"Counter":
            var counter = new Counter(e.item, e.result);
        ...
        case"Objective":
            var counter = new Counter(e.item, e.result);
            cPanel.panels.Objectives.tiles.push(counter);
            break;
        case"Leader":
            var counter = new Counter(e.item, e.result);
            cPanel.panels.Leaders.tiles.push(counter);
            break;
        case"Flag":
            var flag = new Counter(e.item, e.result);
            cPanel.panels.Flags.tiles.push(flag);
            break;
        case"Misc":
...
createjs.Ticker.framerate = 50, createjs.Ticker.addEventListener("tick", loadertick), function () {
    ...
    function e(e, t) {
        this.Container_constructor(), this.x = e, this.y = t, this.curX = e, this.panels = {
            ...
            Objectives: new createjs.Container,
            Leaders: new createjs.Container,
            CC: new createjs.Container(),
            Flags: new createjs.Container(),
            Misc: new createjs.Container
        };
        for (var i in this.panels) this.panels[i].tiles = [];
        this.activePanel = this.panels.Units, this.bounds = new createjs.Graphics.RoundRect(0, 0, e - t / 2, element.height - 1.5 * t, 15, 15, 15, 15), this.border_length = new createjs.Graphics.LineTo(0, 0), this.border_height = new createjs.Graphics.LineTo(0, 0)
    }
