...
Grid.prototype.pixelToIntegerQR = function (x, y, scale) {
  var q, r;
...
Grid.prototype.hexCorner = function (center, i) {
  var angle_deg = 60 * -i;
  var angle_rad = (Math.PI / 180) * angle_deg;
  return {x:center.x + this.tileSize * -Math.cos(angle_rad),
          y:center.y + this.tileSize * -Math.sin(angle_rad)};
};

Grid.prototype.angle = function (center, p1) {
  //angle = atan2(mouseY - cirleCenterY, circleCenterX - mouseX);
  return Math.atan2(p1.y - center.y, center.x - p1.x);
};

Grid.prototype.triangle = function (center, p1) {
  //var angle_rad = this.angle(center, p1);
  return Math.ceil(this.angle(center, p1)/(Math.PI/3));
};
